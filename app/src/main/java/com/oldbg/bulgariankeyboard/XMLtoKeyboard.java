...
public class XMLtoKeyboard extends Keyboard {
    ...
    public XMLtoKeyboard(Context context, int xmlLayoutResId) {
        super(context, xmlLayoutResId);
        Log.d("BG KEYBOARD",String.valueOf(xmlLayoutResId));
    }

    public XMLtoKeyboard(Context context, int layoutTemplateResId,
    ...
    public Key getmSpaceKey() {
        return mSpaceKey;
    }

    public Key getmEnterKey() {
        return mEnterKey;
    }
    ...
    protected Key createKeyFromXml(Resources res, Row parent, int x, int y,XmlResourceParser parser) {
        key = new  Key(res, parent, x, y, parser);
        if (key.codes[0] == 10) {
            mEnterKey = key;
        } else if (key.codes[0] == ' ') {
            mSpaceKey = key;
        ...
        } else if (key.codes[0] == KeyboardInputView.KEYCODE_LANGUAGE_SWITCH) {
            ...
            //key.icon = new TextDrawable(key,85, Typeface.SANS_SERIF,false, Color.BLACK);
        }
        return key;
    }
