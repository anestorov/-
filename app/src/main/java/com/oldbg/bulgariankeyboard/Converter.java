 ...
 * @автор Петър Неделев e-mail: nedelevbg@abv.bg
 */
...
public class Converter {
    ...
    private final String[] exclusionWords1 = {"празник","празнич","сърц","сърч","нужни","въстан","въстава"};
    private final String[] exclusionWords2 = {"нишк","овошк"};
    private final String[] correctedExclusionWords = {"праздник","празднич","сърдц","сърдч","нуждни","възстан","възстава","нищк","овощк"};
    ...
    private final int exLen1 = exclusionWords1.length;
    private final int exLen2 = exclusionWords2.length;
     ...
     */
    private boolean checkYat(int i, String currentWord, boolean isUpperCase){

        if(currentWord.equals("сте") || currentWord.equals("вещ") || currentWord.equals("лев") || currentWord.equals("нея")) return false;
        if (currentWord.equals("ляв") || currentWord.equals("лява")|| currentWord.equals("лявата")){
            if(isUpperCase){
        ...
        if(currentWord.equals("де") || currentWord.equals("бе") ){
            ...
            return true;
        }else if(currentWord.substring(currentWord.length()-2).equals("те")){
            if(isUpperCase){
        ...
        if(currentWord.length()>=3 && currentWord.substring(0,3).equals("пре") || currentWord.length()>=3 && currentWord.substring(0,3).equals("две")){
            if(isUpperCase){
                words[i] = words[i].replaceFirst("Е","Ѣ");
            }else{
                words[i] = words[i].replaceFirst("е","ѣ");
    ...
    private boolean checkFemaleThe(int i, String currentWord, boolean isUpperCase){
        if(currentWord.substring(currentWord.length()-3).equals("тта") || currentWord.substring(currentWord.length()-3).equals("щта") ){
            if(isUpperCase){
     ...
     */
    private boolean checkExclusionWords1(int i, String currentWord, boolean isUpperCase){
        for (int k=0; k<exLen1; k++){
            int exIndex = currentWord.indexOf(exclusionWords1[k]);
            if(exIndex != -1){
                if(isUpperCase){
                    words[i] = words[i].replaceFirst(exclusionWords1[k],correctedExclusionWords[k].toUpperCase(loc));
                }else{
                    words[i] = words[i].replaceFirst(exclusionWords1[k],correctedExclusionWords[k]);
                }
                return true;
            }
        }
     ...
     * Проверява дали думата съдържа корен, който е имал различно изписване
     */
    private boolean checkExclusionWords2(int i, String currentWord, boolean isUpperCase){
        for (int k=0; k<exLen2; k++){
            int exIndex = currentWord.indexOf(exclusionWords2[k]);
            if(exIndex != -1){
                if(isUpperCase){
                    words[i] = words[i].replaceFirst(exclusionWords2[k],correctedExclusionWords[k+exLen1].toUpperCase(loc));
                }else{
                    words[i] = words[i].replaceFirst(exclusionWords2[k],correctedExclusionWords[k+exLen1]);
                }
                return true;
            }
        }
    ...
    /**
     * Конвертира масива от думи words
     */
    public void checkWords(){
        for (int i=0; i<words.length; i++){
            ...
            try {
                ...
                checkYat(i, currentWord, isUpperCase);
                checkVS(i, currentWord, isUpperCase);
                checkFemaleThe(i, currentWord, isUpperCase);
                checkExclusionWords1(i, currentWord, isUpperCase);
                checkExclusionWords2(i, currentWord, isUpperCase);
    ...
    /**
     * @return списък с конвертираните думи
     */
    public List<String> getWords(){
        List<String> listed = new ArrayList<String>(Arrays.asList(words));
        return listed;
    }

    /**
     * Конвертира само една дума
     * @param word
     */
    public void checkWord(String word){
        try {
            String lastChar = String.valueOf(word.charAt(word.length() - 1));
            boolean isUpperCase = ((!lastChar.equals(lastChar.toLowerCase(loc))) && word.length() > 1);
            lastChar = lastChar.toLowerCase(loc);
            String currentWord = word.toLowerCase(loc);
